...
case class Maestro(args: Args) {
  ...
  def load[A <: ThriftStruct : Decode : Tag : Manifest](delimiter: String, sources: List[String], output: String, errors: String, now: String, clean: Clean, validator: Validator[A]) =
    Load.create(args, delimiter, sources, output, errors, now, clean, validator)
