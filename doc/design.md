...
* Exploratory analytics: This is concept is poorly defined, but essentially means the ability to execute declarative query across various representations in order to find patterns or understand the data, and integration with traditional analytics tools such as SAS
...
* Domain specific languages are important however they should strangle the developer when endge cases arrive (which is extremely often in the real world), therefore we prefer embedded DSLs over external DSLs where possible
...
* SQL is excellent for exploring and working with data, that is already inside a wharehouse or relational store of some kind. It is insufficient for integration purposes, and even if it were sufficient, it would breed coupling over time
...
There are numerous cases where downstream integration is performed. In our case we often transfer data to our actual data wharehouse (Terradata based) and others where we integrate with downstream systems (general ledger, bla, bla, bla). The flow of data can split off at various points in this generic flow. In some cases we transform data from source and transfer it to our Teradata warehouse, in other cases we only transfer our conformed or engineered features to downstream systems.
...
Data lands on the integration node(s), where it is immediatly transfered onto HDFS for further downstream processing. It is also compressed and stored on the NAS and HDFS. As a result there are immediatly 4 copies of the data. 2 copies on the NAS (1 un-compressed and 1 compressed) and similarly 2 copies on HDFS. The source data then gets removed from the NAS and HDFS a day after processing. The compressed copy on the NAS gets removed up 7 days after processing, and finally the compressed data on HDFS gets removed up to 30 days after processing. Note that we aren't 'loosing' data, we are processing the data into our internal parquet tables, which are true to source. These various copies of the data are there to support DR processes, and reduce operational risks. If we introduce a bug into our basic ingest code, we have the source data on hand to re-injest into the 'true to source' tables (even though this is highly unlikely). 
...
Because we use the typesafe API, transformations on narrow (sane) types is simply vanilla scala code. When dealing with really wide structures the API quickly gets unwealdy. We are currently working on some API to make this more tenable, and this section will be updated once that is complete. 
